...
select * from emp
where (job_id, dept_id) = ('PU_MAN', 30); -- and ===> 값을 직접 pair 방식으로 대입해서 행을 선택하는 것은 mysql을 되지만 oracle은 안된다. 즉 DBMS마다 다르다.
...
from (
	...
	having avg(e.salary) >= (select avg(salary) from emp)
	order by 2
) t; -- inline view (from 절에 subquery 사용)
...
select * from emp
where   ifnull(job_id, 'none') != 'ST_CLERK'
 and     salary < (select avg(salary) from emp where job_id='ST_CLERK')
order by salary desc;


select * from emp
where   job_id != 'ST_CLERK'; -- job_id 값이 null인 행도 조회안됨.
...
select salary from emp where job_id = 'SA_REP';
...
-- TODO: 제품(products) 중 한번 이상 주문된 제품 정보 조회
