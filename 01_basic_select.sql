...
select emp_id as "직원ID", -- emp_id에서 조회한 결과를 직원ID로  보여줘라.
		emp_name as "직원 이름",
        hire_date "입사일", -- as 생략가능
        salary "급여",
        dept_name "소속 부서"
from	emp;
...
- 같은 컬럼을 여러번 조회할 수 있다.
************************************** */
select 10 as a, 20 as b; -- select 절에 상수값을 넣어서 확인. 이때 from 생략
select '안녕하세요' as 인사말;
...
select salary,
		comm_pct,
        salary * comm_pct -- 같은 행의 salary와 comm_pct를 연산(*)
from emp;
...
/* *************************************
where 절을 이용한 행 선택 
where 컬럼 연산자 피연산자값 - 컬럼의 값들과 연산 했을 때 True인  행들만 조회 =>> Filter 개념.
************************************* */
...
select emp_id,
        ...
        comm_pct
from emp
where comm_pct between 0.2 and 0.3; -- 0.2,0.3도 포함
...
/*
like -> not like 반대
between a and b -> not between a and begin
...
-- ()를 묶지 않으면 and가 우선이 되어서 (job like '%MAN%' and salary <= 1000) 조건을 만족하거나 (year(hire_date) >= 2008) 조건을 만족하는 행을 선택하게 됨.
