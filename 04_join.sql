...
select  e.emp_id,
        ...
        e.hire_date,
        d.dept_name
from	emp e join dept d on e.dept_id = d.dept_id; -- inner 생략가능.

select count(*)
from	emp e join dept d on e.dept_id = d.dept_id; -- inner 생략가능.

select count(*) from emp; -- 전체 직원수
...
-- TODO 부서별 급여(emp.salary)의 평균을 조회. 부서이름(dept.dept_name)과 급여평균을 출력. 급여 평균이 높은 순서로 정렬. 
-- select dept_id, avg(salary) from emp group by dept_id;
select d.dept_name, 
	   avg(salary) "평균급여"
from   emp e join dept d on e.dept_id = d.dept_id
group by d.dept_id, d.dept_name -- dept_name이 Unique Key 컬럼이 아님(같은 이름의 다른 부서가 있을 수 있다.). 그래서 dept_id와 같이 group by 로 묶어준다.
order by 2 desc;
...
select  e.emp_id,
        ...
        j.job_title, 
        e.salary, 
        concat(s.grade, '등급') as "salary grade",
        d.dept_name
from    emp e join job j on e.job_id = j.job_id
...
select  *
from    emp e left outer join dept d on e.dept_id = d.dept_id    -- source table: emp => emp의 데이터(행)들은 join 조건을 만족하지 않아도 나오도록.
...
-- TODO: EMP 테이블에서 부서_ID(emp.dept_id)가 90 인 모든 직원들의 id(emp.emp_id), 이름(emp.emp_name), 상사이름(emp.emp_name), 
